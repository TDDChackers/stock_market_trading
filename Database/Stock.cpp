...
:_name(name),_path(path)
{
    ...
    if (! _path.empty()) import_database(_path);
}

Stock::~Stock()
{
    std::cout << "Destructiong Stock" << std::endl;
    std::ofstream file;
    ...
    if (!file.is_open())
    {
        std::cout << " file.is_open() = 0" << std::endl;
    }
...
void Stock::insert(const string& data)
{
    ...
    if (id < _timestamp_oldest_close_price)
        _timestamp_oldest_close_price = id;
...
void Stock::import_web_data(time_t from, time_t to)
{
    ...
    if (data[0] == '<') // if HTML prefixes are returned.
