...
class CkMailManW : public CkWideCharBase
{
	...
	CkMailManW(CkMailMan *p) : m_impl(p)
	    {
...
CkStringArrayW *GetBadEmailAddresses(void);
...
bool SendMimeBytes(const wchar_t *w1,const wchar_t *w2,CkByteData &mimeData);
#ifndef WINCE
bool SendMimeBytesQ(const wchar_t *w1,const wchar_t *w2,CkByteData &mimeData);
#endif
...
void SetProgressCallback(CkMailProgress *cb);
...
bool VerifyRecipients(const CkEmailW &email,CkStringArrayW &saBadAddrs);
